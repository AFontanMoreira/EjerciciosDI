...
class Aplicacion(Gtk.Window):
    ...
    def __init__(self):
        super().__init__(title="Ex de uso de Gtk ListBox")
        ...
        etiqueta1 = Gtk.Label(label="Data e hora automatica", xalign=0)
        ...
        commutator = Gtk.Switch()
        commutator.props.valign = Gtk.Align.CENTER
        caixaH.pack_start(commutator, False, True, 0)
        ...
        caixaH = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=50)
        fila2.add(caixaH)
        etiqueta = Gtk.Label(label="permitir actualizacions automaticas", xalign=0)
        ...
        caixaH = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=50)
        fila3.add(caixaH)
        etiqueta = Gtk.Label(label="Formato data", xalign=0)
        ...
        elementos = "Esta es una ListBox ordenada para muestra".split()
        for elemento in elementos:
            listBox2.add(ListBoxRowConDatos(elemento))

        def ordenar(fila1, fila2, datos, notify_destroy):
        ...
        listBox2.set_sort_func(ordenar, None, False)
